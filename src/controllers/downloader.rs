...
pub fn add_new_download(url: &str, output_path: &str) -> i32 {
    ...
    let download_file_obj: DownloadFile = DownloadFile::new(&new_downloader.url, &new_downloader.output, false);
...
pub fn ready_to_download_queue(id: i32) -> bool {
    ...
    let mut new_downloader: Downloads = new_downloader.read_by_id::<Downloads>(id);
...
pub fn pause_download_by_id(id: i32) {
    let pause_download: Downloads = Downloads::blank();
    let mut pause_download: Downloads = pause_download.read_by_id::<Downloads>(id);
...
pub fn get_file_info(id: i32, output_path: &str) -> HashMap<String, String> {
    ...
    let metadata: std::fs::Metadata = metadata(output_path)
        .expect("oh well some race condition occured.file not found or anything similiar.");
